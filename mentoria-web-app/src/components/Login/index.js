...
export default function Login() {
  ...
  function checkSenha() {
    dispachsenha({
      type: 'FAZER_LOGIN',
      senha: 'Você digitou a senha corretamente',
    });
  }

  return (
    <div>
      {atividadeusuario.map(user => (
        <label key={user}> {user} </label>
      ))}
      ...
      {atividadesenha.map(senha => (
        <label key={senha}> {senha} </label>
      ))}
      ...
      <div>
        <label htmlFor="pass"> Senha (mínimo de 8 letras ou números):</label>
        <input
          type="password"
          id="pass"
          name="password"
